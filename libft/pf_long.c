...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/08/01 13:30:53 by elopez            #+#    #+#             */
/*   Updated: 2017/08/25 20:02:36 by eLopez           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	get_len(t_flags *flag, long num)
{
	int		len;
	long	prefix;
...
int			pf_long(t_flags *flag, va_list *ap)
{
	...
	sign = 0;
	u.lval = va_arg(*ap, long);
	s = ft_lltoa((long long)u.lval);
	len = get_len(flag, u.lval);
	if (u.lval < 0 || (flag->sign | flag->space))
		sign = 1;
	if ((flag->space | flag->sign) && u.lval >= 0)
