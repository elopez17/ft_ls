...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/08/01 13:23:21 by elopez            #+#    #+#             */
/*   Updated: 2017/08/25 20:06:47 by eLopez           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static size_t	get_len(t_flags *flag, intmax_t num)
{
	int			len;
	intmax_t	prefix;
...
int				pf_intmax_t(t_flags *flag, va_list *ap)
{
	...
	sign = 0;
	u.imax = va_arg(*ap, intmax_t);
	s = ft_imttoa(u.imax);
	len = get_len(flag, u.imax);
	if (u.imax < 0 || (flag->sign | flag->space))
		sign = 1;
	if ((flag->space | flag->sign) && u.imax >= 0)
